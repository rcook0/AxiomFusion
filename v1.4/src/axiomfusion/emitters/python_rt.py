...
def emit(mod: IRModule) -> str:
    lines: List[str] = []
    lines.append('"""AxiomFusion v0.4 - Python skeleton (bar + tick + state + reducers)"""')
    lines.append("import pandas as pd")
    lines.append("")
    lines.append("def run(df: pd.DataFrame):
    # NOTE v0.5: use axiomfusion.runtime.run_backtest for policy-driven execution realism.
")
    lines.append("    state = {}")
