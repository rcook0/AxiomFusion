...
## v1.5
- Semantic contracts beyond caps: targets declare semantic support (tick, MTF alignment, history indexing, state persistence, trade model).
- Validator infers required semantics from source and fails with explicit error codes.
- Conformance fixtures include must-fail cases with expected codes.

## v1.8
- Python backtest runtime: series engine, indicator library, MTF resampling, deterministic tick simulation, execution-profile integration, JSONL trace output.
- CLI: `axiomfusionc backtest` and `axiomfusionc trace`.
