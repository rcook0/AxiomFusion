...
def infer_caps(mod: A.Module) -> Set[str]:
    caps: Set[str] = set()
    # structural
    for d in mod.decls:
        if isinstance(d, A.StateDecl):
            caps.add("state")
        if isinstance(d, A.OnTick):
            caps.add("tick")
        if isinstance(d, A.OnBar):
            pass
    # detect trade intents and mtf usage by walking expressions
    def walk_expr(e: A.Expr):
        if isinstance(e, A.Call):
            if e.name.startswith("trade."):
                caps.add("trade")
            if e.name == "series_from":
                caps.add("mtf")
            for _,ex in e.args:
                walk_expr(ex)
    ...
    def walk_stmt(s: A.Stmt):
        if isinstance(s, A.IfStmt):
            walk_expr(s.cond)
            for x in s.block: walk_stmt(x)
        elif isinstance(s, A.CallStmt):
            walk_expr(s.call)
        elif isinstance(s, A.SetStmt):
            walk_expr(s.target); walk_expr(s.expr)
        elif isinstance(s, A.ReduceStmt):
            walk_expr(s.target); walk_expr(s.expr)
    ...
    for d in mod.decls:
        if isinstance(d, (A.InputDecl,)):
            ...
            for f in d.fields: walk_expr(f.expr)
        elif isinstance(d, (A.OnBar, A.OnTick)):
            for s in d.stmts: walk_stmt(s)
    return caps
