...
def emit(mod: IRModule) -> str:
    lines: List[str] = []
    lines.append("// AxiomFusion v0.5 - MQL5 EA skeleton (bar + tick + state + reducers)")
    ...
    lines.append("}")
    lines.append("")
    lines.append("// Execution policies (v0.5): map to EA inputs + helper functions
// - spread/slippage: use SymbolInfoDouble + deviation
// - commission: AccountInfoDouble / broker spec
// - margin check: OrderCalcMargin
// - sessions/news: local time gates + calendar API (future)
// TODO: persist state across restarts + version migration policy.")
    return "\n".join(lines).strip() + "\n"
...
def emit_expr(e: IRExpr) -> str:
    ...
    if isinstance(e, Call):
        ...
        if n == "hl2": return "(iHigh(_Symbol,_Period,1)+iLow(_Symbol,_Period,1))/2.0"
        if n == "ohlc4": return "(iOpen(_Symbol,_Period,1)+iHigh(_Symbol,_Period,1)+iLow(_Symbol,_Period,1)+iClose(_Symbol,_Period,1))/4.0"
        if n == "roc":
            // roc(src,len) approximated for close only
            return "0.0/*roc*/"
        if n in {"wma","highest","lowest","stddev","bb_upper","bb_lower","bb_middle","macd_line","macd_signal","macd_hist"}:
            return "0.0/*lib*/"
        if n == "series_from":
            sym = args[0]
            tf = args[1]
