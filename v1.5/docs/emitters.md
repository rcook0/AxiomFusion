...
## Tick + state notes (v0.3)
- MQL5: state as globals; reset reducer fields on new bar; optionally persist using GlobalVariables/file.
- Pine: tick is not supported; reducer/state updates omitted or approximated.
- Python: tick loop is stubbed per bar; extend with real tick streams later.
