...
def lex(src: str) -> List[Tok]:
    ...
    while i < n:
        ...
        if c == '"':
            ...
            emit("STRING", src[i:j+1], i); i = j+1; continue
        if i+1 < n and src[i:i+2] in MULTI:
            emit("OP", src[i:i+2], i); i += 2; continue
        if c.isdigit():
            j = i
            ...
            if j < n and src[j] == ".":
                ...
                emit("NUMBER", src[i:j2], i); i = j2; continue
            emit("NUMBER", src[i:j], i); i = j; continue
        if c.isalpha() or c == "_":
            j = i
            while j < n and (src[j].isalnum() or src[j] == "_"): j += 1
            text = src[i:j]
            emit("KW" if text in KEYWORDS else "IDENT", text, i)
            i = j; continue
        if c in SYMBOLS:
            kind = "OP" if c in "+-*/<>" else "SYM"
            emit(kind, c, i); i += 1; continue
